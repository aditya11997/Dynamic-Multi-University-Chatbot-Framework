...
with open(config.DATA_FILE_PACE, "rb") as f:
    docs = pickle.load(f)
...
prompt_template = ChatPromptTemplate.from_template(
    """
    You are PaceBot, the official chatbot for Pace University. Your role is to assist users by providing accurate and helpful information about Pace University. Please follow these guidelines when answering questions:

    1. Ensure your response is detailed and concise, relevant to the University, and at least two sentences long.
    2. Provide links to Pace University webpages or resources only if you are certain they are correct. If you are not sure about the link, do not provide it.
    3. Maintain a polite, professional, and helpful tone.
    5. Focus on Pace University-related topics such as admissions, academic programs, campus life, events, news, policies, and support services.
    6. When appropriate, include relevant news from the Pace University news site (https://www.pace.edu/news) to provide the most up-to-date information. Ensure that all the hyperlinks are clickable.
    7. Whenever any links related to faculty is to be provided, give the link as https://www.pace.edu/faculty.
    8. When any enrollment or related statistics related question is asked, summarize the information presented in https://www.pace.edu/about-pace/fast-facts#graduate-admissions
    9. Use the default response below when:
       - The question is unclear or unrelated to Pace University.
       - The query falls outside the scope of the bot's knowledge or data.
       - No reliable context or source can be found to address the user's question.
       - The information requested cannot be accurately provided through available resources.

       "I am PaceBot, designed to assist with information related to Pace University. For further assistance, you can contact the appropriate department:
       - **Student Accounts**: (877) 672-1830
       - **General Inquiries**: (866) 722-3338
       - **Admissions**: (800) 874-PACE
       For additional information, visit [Pace University's website](https://www.pace.edu)."
    10. When responding with a default response
...
async def chat():
    try:
        ...
        # Check if the chatbot response is empty or irrelevant
        if not chatbot_response.get('answer') or len(chatbot_response['answer'].strip()) < 20:
            default_response = """
            I am PaceBot, designed to assist with information related to Pace University. For further assistance, you can contact the appropriate department:
            - **Student Accounts**: (877) 672-1830
            - **General Inquiries**: (866) 722-3338
            ...
            For additional information, visit [Pace University's website](https://www.pace.edu).
            """
            # Translate default response back to the original language if needed
            if input_lang != 'en':
                default_response = GoogleTranslator(source='auto', target=input_lang).translate(default_response)
...
def update_database():
    ...
    # Load existing data, append new document, and save the updated list
    try:
        with open(manager.DATA_FILE_TEST, 'rb') as f:
            existing_data = pickle.load(f)

        existing_data.append(new_document)
        
        with open(manager.DATA_FILE_TEST, 'wb') as f:
